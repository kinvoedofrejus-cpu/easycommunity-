...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      // ---- Dues / cotisations (trésorier only) ----
      if (path === "/api/dues" && method === "POST") {
        const f = forbidIfNotRole(auth, ["tresorier"], origin); if (f) return f;
        const body = await request.json();
        await insertRow(env, "dues", body);
        return json(body, 201, origin);
      }

      // ---- Transactions / comptabilité (trésorier only) ----
      ...
      // L'auteur d'un rapport (même rôle que celui qui l'a créé) peut le
      // mettre à jour librement — utilisé pour remplir/fermer une liste de
      // cotisation par exemple. L'administrateur peut aussi le faire.
      if ((m = path.match(/^\/api\/reports\/([^/]+)$/)) && method === "PUT") {
        const existing = await getRow(env, "reports", m[1]);
        if (!existing) return json({ error: "Rapport introuvable." }, 404, origin);
        const f = forbidIfNotRole(auth, [existing.type, "administrateur"], origin); if (f) return f;
        const body = await request.json();
        const updated = { ...existing, ...body, id: m[1] };
        await updateRow(env, "reports", m[1], updated);
        return json(updated, 200, origin);
      }
